        ...
        <div
            ...
            <div class="text-center">
                ...
                <p class="mt-2 text-lg text-gray-600 dark:text-gray-400">
                    قم بتحديث البيانات الخاصة بالإعلان.
                </p>
            </div>

            <x-forms.form method="POST" action="{{ route('investor.announcements.update', $announcement->id) }}" class="mt-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="6" class="form-input"
                        placeholder="اكتب وصفاً تفصيلياً عن المشروع الذي تبحث عنه...">{{ old('description', $announcement->description) }}</textarea>
                    @error('description')
                ...
                <div class="mt-6">
                    ...
                    <div x-data="{
        search: '',
        selectedCategories: {{ $announcement->categories->toJson() }},
        showDropdown: false,
            ...
            this.selectedCategories = this.selectedCategories.filter(c => c.id !== category.id);
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="location" name="location" value="{{ old('location', $announcement->location) }}"
                    ...
                    <div>
                        ...
                        <input type="number" id="budget" name="budget" value="{{ old('budget', $announcement->budget) }}" class="form-input"
                    ...
                    <div>
                        ...
                        <input type="date" id="start_date" name="start_date" value="{{ old('start_date', $announcement->start_date->format('Y-m-d')) }}" class="form-input" min="{{ now()->toDateString() }}">
                        @error('start_date')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" id="end_date" name="end_date" value="{{ old('end_date', $announcement->end_date->format('Y-m-d')) }}" class="form-input" min="{{ now()->toDateString() }}">
                        @error('end_date')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    </div>
